...
df_communes = pd.read_csv("data/communes.csv", sep=",",
    dtype={"code_commune_INSEE" : str, "code_departement" : str, "code_region": str})

df_departements = pd.read_csv("data/departements-france.csv", sep=",")

df_candidats = pd.read_csv('data/candidats-2026.csv', sep=";", dtype={'Code département': str, 'Code circonscription': str, 'CC': str})

df_insee = pd.read_csv('data/insee-communes.csv', sep=";", dtype={'CODGEO': str, 'REG': str, 'DEP': str})
...
df_departements['code_departement'] = df_departements['code_departement'].str.zfill(2)
...
df_regions.to_csv(f"clean/{dept}-code_region.csv", sep=",", index=False, quoting=QUOTE_NONNUMERIC)

df_departements.to_csv(f"clean/{dept}-departements.csv", sep=",", index=False, quoting=QUOTE_NONNUMERIC)
